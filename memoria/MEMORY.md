...
- [postgres-servico-nao-pg_ctl](licoes/postgres-servico-nao-pg_ctl.md) — login 500 `P1001` é banco parado, não senha; sobe por `Start-Service` elevado (UAC via `-Verb RunAs`), nunca `pg_ctl` manual: colide com o serviço e se desliga sozinho. Tarefa S4U (sem janela piscando) também exige elevação.
